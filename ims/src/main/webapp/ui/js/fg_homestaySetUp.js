...
function doSetUp(){
	...
	console.log(obj.jsrsuOnlineDepositRatio)
	...
	console.log(onlineDepositPrcent)
	$.ajax({
...
//删除图片
function removeMsPic(){
	var photos = $('.contFile');
	if(photos.length == 0){
		$.messager.alert('消息','没有图片可以删除',"error");
	}else{
		$('#removeMsPicture').html('请选择要删除的图片').show();
		$('.picturecheck').show();
		$('#doRemoveMsPic').show();
	}
}
//执行删除图片
function doRemoveMsPic(){
	var arr = 0;
	var path = '';
