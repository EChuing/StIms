...
//加载库房数据
function storeroomData(page, type){
	var startNum = (parseInt(page) - 1) * 20;
	var endNum = 20;
	var addCity = '库房';
	var searchVirtualName = $("#searchVirtualName").val();
	...
	$.post("../queryProjectCommon.action", {
		...
		if (data.code<0) {
			...
			if(page==1){
				notCountPage(0, 0 ,"storeroomData","virtual");
			}else{
				notCountPage(page, 0 ,"storeroomData","virtual");
			}
		} else {
			// if (page == 1 && type == 0) {
			...
			if(data.body.length<endNum){
				notCountPage(page, 2 , "storeroomData","virtual");
			}else{
				notCountPage(page, 1 , "storeroomData","virtual");
			}
...
function validate(type){ 
	if($("#virtualName").val() == ""){
		$("#error").val("请输入库房名称"); return;
	}
//	if($("#initialAmount").val() == ""){
//		$("#error").val("请输入初期金额"); return;
//	}else if($("#calibrationAmount").val() == ""){
//		$("#error").val("请输入校准金额"); return;
//	}
//	var result = _isIntOrDouble.test($("#initialAmount").val());
//	if(!result){
//		$("#error").val("初期金额填写错误"); return;
//	}
//	result = _isIntOrDouble.test($("#calibrationAmount").val());
//	if(!result){
//		$("#error").val("校准金额填写错误"); return;
//	}
	if(type==0){
		doAddVirtual();
	}else if(type==1){
...
//		$("#initialAmount").val(row.houseSellingPrice);
//		$("#calibrationAmount").val(row.unitPriceRent);
