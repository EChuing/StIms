...
function queryDeviceInfo() {
    ...
    $.post("../selectThisHouseDeviceID.action", {
        ...
        if (data.code < 0) {
            ...
            //异步加载状态，这里有个问题，每查一个状态后都会重新加载列表，之前选中的行会被清除，如果用户已经打开了控制设备的弹窗，点击控制按钮会报错
            for (var i in data) {
                var devJson = data[i];
                var eleAmount;
                var cmdCount;
                var signalQuality;
                if (devJson.devBrandId == 10 && devJson.devFirstType == 3 && devJson.devSecondType == 3) {
...
function queryOfficeDevice() {
    var row = $('#trusteeshipDg').datagrid('getSelected');
    var hsId = row.hsId;
    $.post("../queryOfficeDevice.action", {
        jhdHsId: hsId
    }, function (data) {
        if (data.code < 0) {
            $('#deviceInfoOfficeTable').datagrid({
                data: [],
                view: myview,
                emptyMsg: data.msg
            });
            ...
            for (var i in data) {
                            ...
                            if (data[j].devSpare2 == data1.body.code) {
                                if (data1.code == 1) {
                                    ...
                                    data[j].devStatus = "<div style='float:left;margin:0 0 0 10px;'><img src='img/battery.png' width='18px' style='float:left;margin:0 0 0 10px;'><span style='float:left;'>" + eleAmount + "%</span></div>"
                                        + "<div style='float:left;margin:0 0 0 10px;'><img src='img/signal.png' width='18px' style='float:left; margin:0 0 0 10px;'><span style='float:left;'>" + signalQuality + "格</span></div>"
                                        + "<div style='float:left;margin:0 0 0 10px;'><img src='img/frequency.png' width='18px' style='float:left; margin:0 0 0 10px;'><span style='float:left;'>" + cmdCount + "次</span></div>";
                                    $("#deviceInfoOfficeTable").datagrid("loadData", data);
                                } else {
                                    data[j].devStatus = "暂无数据";
                                    $("#deviceInfoOfficeTable").datagrid("loadData", data);
                                }
                ...
                if (devJson.devBrandId == 20 &&(
                    (devJson.devFirstType == 1 && devJson.devSecondType == 1)  ||
                    (devJson.devFirstType == 3 && devJson.devSecondType == 22) ||
                    (devJson.devFirstType == 3 && devJson.devSecondType == 23) ||
                    (devJson.devFirstType == 3 && devJson.devSecondType == 40) ||
                    (devJson.devFirstType == 4 && devJson.devSecondType == 4)  ||
                    (devJson.devFirstType == 6 && devJson.devSecondType == 6)  ||
                    (devJson.devFirstType == 2 && devJson.devSecondType == 25) ))  {//云海电箱
                    $.post("../queryDeviceStatus.action", {
                        devAuthId: devJson.devAuthId,
                        devId: devJson.id
                    }, function (data1) {
...
//	var postUrl = "http://localhost/device/api";
    ...
    switch (brandId) {
        ...
        case 9://云海物联-所有设备
            ...
            break;*/
        case 10://电易-门锁
            if (username == '' || password == '' || authId == '' || authSecret == '') {
                myTips('信息填写不完整', 'error');
                return;
            }
            postJson = {
    ...
    switch (type) {
        ...
        case 5: // 远程开锁
            if (brandId == 2) {
                ...
                if(devFirstType == 24 && devSecondType == 33){
                    postJson.sn = devAuthId;
                    postJson.mac = "";
                    postJson.instruction = "远程开锁";
                    postJson.brandId = brandId;
                    postJson.deviceKey = devSn;
                    postJson = JSON.stringify(postJson);
                }
            }
            break;
        ...
        case 7: // 设置自定义密码
            ...
            }else if(brandId == 22 && devSecondType == 33){
                postJson.instruction = "插入临时密码";
                postJson.brandId = brandId;
                postJson.devId = devId;
                postJson.coId = coId;
                postJson.password = password;
                postJson.devUsername = devUsername;
                postJson.startDate = startDate;
                postJson.endDate = endDate;
                postJson.jhdHsId = jhdHsId;
                postJson.operateType = operateType;
                postJson = JSON.stringify(postJson);
            }
            break;
...
function doBindDevice() {
    ...
    for (var i in rows) {
        ...
        jhdDeviceIdJson.push({
            ...
            devAuthId:rows[i].devAuthId,
            devFirstType:rows[i].devFirstType,
            devSecondType:rows[i].devSecondType,
        });
    }
    showLoading();
...
function removeHsDevice() {
    ...
    var hs = $('#trusteeshipDg').datagrid('getSelected');
