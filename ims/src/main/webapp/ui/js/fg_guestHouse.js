...
//钟点房入住时间
var hour_time =["06:00","07:00","08:00","09:00","10:00","11:00","12:00","13:00","14:00","15:00","16:00","17:00","18:00","19:00"];
...
function checkInRoom(){
    ...
    var sectionType=$('#handle3 .sectionType').val();			//获取选择房间类型户型
    ...
    $.ajax({
        ...
        type:"post",
        data:{
            hsSectionType   : sectionType,
            hsRoomType      : tRoomType,
            hsDirtyHouse    : 0,
            hsLeaseState    : '短租房'
        },
        success:function(data){
            if(data.code > 0){
                data = data.body
                for(var i in data){
                    var hsRoomType=data[i].hsRoomType;			//获取房间类型
                    var hsId=data[i].hsId;						//获取未租房间Id
                ...
                $('#retainRoomNum').html("空房数量: 0");		//显示空房数量
                $('#numSetBerInput').val(0)		//将空房数量隐藏起来
            }
        }
    });
}
...
/*function typeOccupancy(){
    ...
    if(typeOccupancy=="钟点客房"){//判断入住类型等于钟点房
        ...
        var totalHourPrice =Number(hsTimePrice)*hourRoom;
        $('#dayPrice').val( hsTimePrice);	//赋值给日均价格
        ...
        //计算应付金额
        var deposit = parseFloat($('#houseDeposit').val());
        var totalPrice = (deposit +  parseFloat(amountPayable)).toFixed(2);

        $("#totalDay").val(days);                   //总天数
        $('#dayPrice').val(averagePrice);           //日均价格
        $('#totalHousingPrice').val(amountPayable); //总房价
        $('#amountPayable').val(totalPrice);        //应付金额
        $('#totalPrice').val(totalPrice);           //折后金额
        $('#moneyText').html(totalPrice);           //金额显示
...
function querySourceInfo(){
    ...
    if(houseList != null){
        ...
        $('.shortRentState .totalNum0').html('（0）');
    }
    ...
    $.ajax({
        ...
        success:function(result){
            if(result.code == 1){
                ...
                $('#fastCheckInApartment').html(sectionType);
...
function checkInRoomNum(){
    ...
    //获取选择的户型
    var sectionType = $('handle1 .sectionType').find('option:selected').val();
    ...
    $.ajax({
        ...
        data:{
            hsSectionType   : sectionType,
            hsRoomType      : fastCheckInHomeType,
            hsDirtyHouse    : 0,
            hsLeaseState    : '短租房'
        },
        success:function(data){
            ...
            if(data.code > 0){
                ...
                for(var i in data){
                    ...
                    var canCheckIn=checkTime(hsId,startTime,endTime,0);	//调用查询空房间Id
                    if(canCheckIn){
                        var hsAddCommunity = data[i].hsAddCommunity;
                ...
                $("#checkInList1").html("");
                $("#quickCheckIn1").val("");
                $("#checkInRoomNum").html("空房数量：0");
                return;
            }

            _checkInRoom =roomList;				//把适合房间放进去全局变量
            if(fastCheckInHomeType != '' || sectionType != ''){
            ...
            if(roomList.length>0){
                ...
                var randomBuilding=roomList[n].hsAddBuilding == null?"":data[i].hsAddBuilding;
...
function checkInKey1(){
    ...
    var quickCheckIn1=$('#quickCheckIn1').val();
    var obj  = {};
    console.log(_checkInRoom)
    console.log(quickCheckIn1)
...
function openAddShortRent(resObj,startDate,endDate){
    ...
    for(var i=0;i<length-1;i++){
        ...
        if(min <= max){
            ...
            $("#hourStartSelect").append("<option style='width:36px' value='" + beginTime + "'>"+beginTime+"</option>")
        }
    };
...
function openCheckIn(){
    ...
    var weekdayResult = checkWeekend(startTime,endTime);
    var weekday = weekdayResult.weekDays;
    $('#fontTitle1').html("<div style='margin:5px 0 0 0'>所选时间段内含有"+weekday+"天周末，建议日均价格为</div>")
...
function doSceneCheckIn(type,house){
    ...
    var jsrcActualOccupancyTime=new Date().format('yyyy-MM-dd hh:mm:ss');	//实际入住时间
    var jsrcEndTime = $('#endDate').val();									//合约结束时间
    var jsrcTypeOccupancy=$('#typeOccupancy').val();						//入住类型
...
function openCheckOut(data){
    ...
    //已经入住的
    if(data.jsrcState == "已住" || data.jsrcState == "退房"){
        var pop = JSON.parse(data['popJson'].getRealJsonStr());
    ...
    if(data.jsrcState == "保留" || data.jsrcState == "预定" || data.jsrcState == "退定中"){
        if(data.jsrcPeople != null && data.jsrcPeople != ''){
            var pop = JSON.parse(data.jsrcPeople.getRealJsonStr());
            evaluation(1,pop[0])
            $('#customerInformation').datagrid('loadData', pop);
            $('#popName').val(pop[0].popName);
        }
    ...
    //总共金额 押金+总房价
    var totalPrice1 = (data.jsrcDeposit + data.jsrcAmountPayable).toFixed(2);
    $('#totalPrice1').val(totalPrice1);
    //折后金额
    $('#amountOfDiscount').val(totalPrice1);
...
function checkOutShortRent(type,payType){
...
function initInfo(){
    ...
    var searchHsAddBuilding = $("#searchHsAddBuilding").val();
    $.ajax({
        type:"post",
        url:"../selectShortRentHouse.action",
        data:{
            ...
            hsAddBuilding           : searchHsAddBuilding,
            hsLeaseState			: "短租房",
        },
        dataType:"json",
        success:function(data){
...
function changeHouseMoney(){
    ...
    var newHouseAddress = $('#canChangeHouseList').find('option:selected').text();

    //合约时间
    var jsrcBeginTime = $('#jsrcBeginTime').val();
    var jsrcEndTime = $('#jsrcEndTime').val();

    var totalDays = parseFloat($('#jsrcTotalDays').val());
    var weekdayResult = checkWeekend(jsrcBeginTime,jsrcEndTime); //计算周末
    var weekDays = weekdayResult.weekDays;

    for(var i in house_list_arr){
        var address = house_list_arr[i].hsAddCommunity + house_list_arr[i].hsAddBuilding + house_list_arr[i].hsAddDoorplateno
        if(address == newHouseAddress){
            var hsId = house_list_arr[i].hsId;
            var dailyRent = house_list_arr[i].hsDailyRent;
            var hotDailyRent = house_list_arr[i].hsHotDailyRent;
            //押金
            var deposit = 0.0;
            if(setUp.jsrsuDepositSetType != ""){
                var jsrsuDepositSetType = JSON.parse(setUp.jsrsuDepositSetType);
                if(jsrsuDepositSetType.type == 0){
                    deposit = Math.round((dailyRent/100)+0.49)*100;	//押金
                    deposit = deposit.toFixed(2)
                }else{
                    deposit = jsrsuDepositSetType.depositMoney
                }
            }
            var newTotalPrice = deposit + dailyRent*(totalDays-weekDays) + weekDays*hotDailyRent;
            newTotalPrice = newTotalPrice-oldTotalPrice > 0 ? newTotalPrice-oldTotalPrice : 0.00;
            $('#changeHouseMoney').val(newTotalPrice)
        }
    }
...
function openCash(type,orderType){
    ...
    if(orderType == 1){
        $("#payType").attr("onclick","canCheckIn("+type+")").html(title);
    }else if(orderType == 2){
        $("#payType").attr("onclick","doCheckIn("+type+")").html(title);
    }else if(orderType == 3){
        $("#payType").attr("onclick","checkOutShortRent(0,"+type+")").html(title);
    }else if(orderType == 4){
        $("#payType").attr("onclick","checkOutShortRent(1,"+type+")").html(title);
    }else if(orderType == 5){
...
function authSubmit() {
    ...
    if(type == "远程授权"){
        ...
        var roomType = $("#typeOccupancy").val();
        var totalHousingPrice =$("#totalHousingPrice").val();
        ...
        $.ajax({
            ...
            success:function (result) {
                ...
                if(result.code == 1){
                    ...
                    $.post("../selectBySelective.action",
                        {jtoOrderId:jsrcHsIdtwo},function (result) {
