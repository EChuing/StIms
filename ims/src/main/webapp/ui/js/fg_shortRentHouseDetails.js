...
function houseInfoDlg(){
	$('#houseInfoDlg').dialog({
		title : _title_address+" 客房详情",
		top : getTop(580),
		left : getLeft(950),
		width : 830,
		height : 550,
		...
		$.ajax({
				...
				success : function(result){
					...
					if(result.code < 0){
						...
						for(var i in data){
							console.log(data[i]);
							var needDevId = (data[i].devId == 25||data[i].devId == 49);
							if(data[i].devBrandId == 20 ){ //20为云海的设备
								if(needDevId){
									console.log(data[i].devNickname)
									var html = '' +
										'<div class="lock">'+data[i].devNickname+'<input type="checkbox" checked="checked" first_dev_id="'+data[i].devFirstType+'" second_dev_id="'+data[i].devSecondType+'" data-name="'+data[i].devNickname+'" value="'+data[i].id+'"/></div>';
									htmls += html;
									break;
								}
							}
							...
							success : function(result){
								if(result.code < 0){
									...
									for(var j in peopleList){
										var html = '<option value="'+peopleList[j].popId+'" data-delineTime="'+delineTime+'">'+peopleList[j].popName+'</option>'
										htmls += html;
									}
...
function doSaveAuthDoorCard(){
	...
	$(".lock input").each(function(){
		if(this.checked){
			...
			lockName += " ";
			devFirstType = $(this).attr("first_dev_id");
			devSecondType = $(this).attr("second_dev_id");
      }
	})
	...
	console.log(devIdftId+"====="+devIdstId);
	var inseratData = {
			jdcHsId : jdcHsId,
