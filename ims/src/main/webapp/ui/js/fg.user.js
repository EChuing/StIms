...
function updateUserDlg() {
	...
	$.post("../selectSuPermissionsId.action", {userId :_loginUserId}, function(data) {
		if(data.code<0){
			myTips(data.msg,"error");
			return;
		}else{
			if(data.body == 1 ) {
				if (row.suFrozen == 0) {
					$("#suFrozenDiv").css("display", "block");
				} else {
					$("#suFrozenDiv").css("display", "none");
				}

			}else {
				//myTips("请联系，管理员","error");
			}
		}
	});
	$("#addUserDlg").dialog('open');
...
function doPushingCard(data1) {
	...
	console.log(doorCardNum);
	var personType = $('#add_userType').val();
...
/* ---------subFrozenTrueOrNot冻结状态()--------- */
function updateSubFrozenTrueOrFalse(num){
	var row = $('#userDg').datagrid('getSelected');
	if(!row){
		myTips("请选择要进行操作的用户！","error");
		return;
	}
	showLoading();
	var registrantName =_loginUserName;

	var suFollowMsg = registrantName + "冻结了该账户;";

	$.post("../updateSubFrozenTrueOrFalse.action", {
			userId 			: row.userId ,
			suFrozen 		: num,
			registrantName 	: registrantName,
			sufollow		: suFollowMsg,
	}, function(data) {
		if(data.code<0){
			myTips(data.msg,"error");
			return;
...
function formatSubFrozenTrueOrNot(value, row, index) {
	if (row.suFrozen == 0 ) {
		return "<a style='text-decoration:none;color:red;'>冻结<a>";
	} else{
