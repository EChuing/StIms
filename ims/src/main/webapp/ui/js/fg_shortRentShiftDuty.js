...
//分页统计总条数
function getshiftWorkPageCount(page){
    var pageSize = 20;
    $.ajax({
        ...
        if (data.code < 0 || data.body[0].totalNum == 0){
            ...
            getCountData(0,countJson,pageSize,page,"shiftWork",0);
        } else {
            data=data.body;
            var countJson = {
                totalNum: data[0].totalNum,
            };
            getCountData(1,countJson,pageSize,page,"shiftWork",0);
        }
    }});
}
    ...
    $.ajax({
        ...
        success:function(result){
            if (result.code<0) {
                ...
                if(page==1){
                    notCountPage(0, 0 ,"getAllShortRentShiftDuty","shiftWork");
                }else{
                    notCountPage(page, 0 ,"getAllShortRentShiftDuty","shiftWork");
                }
                $('#shiftWorkDg').datagrid({
                    data : [],
                    view : myview,
                ...
                if(data.length<endNum){
                    notCountPage(page, 2 , "getAllShortRentShiftDuty","shiftWork");
                }else{
                    notCountPage(page, 1 , "getAllShortRentShiftDuty","shiftWork");
                }
