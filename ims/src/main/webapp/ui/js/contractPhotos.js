...
function doRemoveRent(){
	...
	if(arr > 0){
		...
		$("#imgWrapperRent input[name='other']:checked").each(function() { // 遍历选中的checkbox
			if(imgPaths.search($(this).parent().children("a").attr('href').split("?")[0]) == -1){
				flag = false;
			}
		...
		if(flag){
			$.post("../deleteCsSalesClientContract.action",{
				csContractId : row.csContractId,
				csImgPath : path
			}, function(data) {
				if (data <0 || data=='') {
					myTips('删除失败！', 'error');
					...
					myTips('删除成功！', 'success');
					$('#renterRenewalImgDlg').dialog('close');
				}
			});
		}else{
