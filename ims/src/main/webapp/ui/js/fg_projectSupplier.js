...
//		houseSellingPrice : initialAmount,
//		unitPriceRent : calibrationAmount,
	}, function(data){
		if(data.code < 0){
			myTips(data.msg, "error");
			return;
		}
//		$.post("../calBalanceOfVirtual.action", {
//			houseCoding : houseCoding,
//			houseSellingPrice : initialAmount,
//			unitPriceRent : calibrationAmount,
//		}, function(acountData) {
...
function queryRepair(page,type){
	...
	$.post("../queryTaskCommon.action", {
		...
		if (data.code<0) {
			...
			for (var i in data) {
				data[i].repReportingTime = formatTime(data[i].repReportingTime,1);
			}
