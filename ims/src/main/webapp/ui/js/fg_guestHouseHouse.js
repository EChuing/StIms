...
function addShortRentHouse(){
    ...
    if(addShortRentDoorplateno.length>0&&addShortRentDoorplateno.length<3){
        var addShortRentFloor=addShortRentDoorplateno.substring(0, 1);
    }else{
        var addShortRentFloor=addShortRentDoorplateno.substring(0, addShortRentDoorplateno.length-2);
    }
...
//根据房型来修改日均房价和高峰价格
function shortHouseMoney(){
    console.log(planInfo)
    var houseType = $('#addShortHouseType').val();
    for(var i in planInfo){
        if(planInfo[i].roomType == houseType && planInfo[i].jsrrtpPlanPackage != ""){
            var planPackage = JSON.parse(planInfo[i].jsrrtpPlanPackage);
//			$('#addHsDailyRent').val(parseFloat(planPackage[0].normalPrice).toFixed(2));
//			$('#addHsHotDailyRent').val(planInfo[i].hotDailyPrice);
        }
    }
}