...
//分页统计总条数
function getchannelPageCount(page){
	var pageSize = 20;
	...
	$.ajax({
		...
		success:function(data) {
			if (data.code < 0 || data.body[0].totalNum == 0) {
				...
				getCountData(0, countJson, pageSize, page, "channel", 0);
			} else {
				data = data.body;
				var countJson = {
					totalNum: data[0].totalNum,
				};
				getCountData(1, countJson, pageSize, page, "channel", 0);
			}
		}
	});
}
	...
	$.ajax({
		...
		success:function(data){
			if(data.code == 1){
                ...
                // }
				if(channelInfo.length<endNum){
					notCountPage(page, 2 , "getChannelInfo","channel");
				}else{
					notCountPage(page, 1 , "getChannelInfo","channel");
				}
				...
				// sourcePage(0, 0, 1);
				if(page==1){
					notCountPage(0, 0 ,"getChannelInfo","channel");
				}else{
					notCountPage(page, 0 ,"getChannelInfo","channel");
				}
				$('#channelTable').datagrid({
					data : [],
					view : myview,
...
function getPricePlanInfo(page){
	...
	var jppState = 1;
	if(!$('#pricePlanDlg').is(":hidden")){
		jppState = $('#pricePlanState').val();
	}

	console.log("方案状态："+jppState)
	$.ajax({
