...
function populationDetailedDlg(row){
    console.log(row);
	$('#pop_telephone').val(row.popTelephone);//联系电话
	$('#pop_marriage_state').val(row.popMarriageState);//婚姻状态
	$('#pop_occupation').val(row.popOccupation);//职业
	$('#pop_degree_education').val(row.popDegreeEducation);//文化程度
	$('#pop_inner_credit_level').val(row.popInnerCreditLevel);
	$('#pop_outer_credit_level').val(row.popOuterCreditLevel);
	$('#pop_name_remark').val(row.popNameRemark);//备注
	$('#jsrrCustomerType').val(row.jsrrCustomerType);
	$('#pop_memberBalance').val(row.popMemberBalance);
	if(row.popIdcardJson !='' && row.popIdcardJson != null){
		...
		console.log('identityInformation'+identityInformation);
		...
		$('#pop_name').val(row.popName);//客户姓名
		$('#pop_idcard_type').val(row.popIdcardType);//证件类型
		$('#pop_sex').val(row.popSex);//性别
		$('#pop_nation').val(row.popNation);//民族
		$('#pop_idcard_address').val(row.popIdcardAddress);//户籍地址
		$('#pop_birth').val(row.popBirth);//生日
		$('#pop_idcard').val(row.popIdcard);//证件号码
	}
