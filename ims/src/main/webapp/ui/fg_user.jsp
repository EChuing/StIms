...
<title>用户管理</title>
<link
	href="http://pic-static.fangzhizun.com/easyui/themes/default/easyui.css"
	rel="stylesheet">
<link href="http://pic-static.fangzhizun.com/easyui/themes/icon.css"
	rel="stylesheet">
<link href="http://pic-static.fangzhizun.com/css/mytips.css"
	rel="stylesheet">
<link href="http://pic-static.fangzhizun.com/css/colorbox.css"
	rel="stylesheet">
<link href="http://pic-static.fangzhizun.com/css/webuploader.css"
	rel="stylesheet">
<link href="css/icon.css" rel="stylesheet">
<link href="css/style.css" rel="stylesheet">
<link href="css/upload.css" rel="stylesheet">
<script
	src="http://pic-static.fangzhizun.com/jquery/2.1.4/jquery.min.js"></script>
<script
	src="http://pic-static.fangzhizun.com/jquery-qrcode/1.0.0/jquery.qrcode.min.js"></script>
<script src="http://pic-static.fangzhizun.com/js/jquery.easyui.min.js"></script>
<script src="http://pic-static.fangzhizun.com/js/jquery.colorbox-min.js"></script>
<script
	src="http://pic-static.fangzhizun.com/js/jquery.colorbox-zh-CN.js"></script>
<script
	src="http://pic-static.fangzhizun.com/js/webuploader.html5only.min.js"></script>
<script
	src="http://pic-static.fangzhizun.com/My97DatePicker/WdatePicker.js"></script>
...
<body>
	...
	<div id="DataGridStorefront" style="width: 20%; height: 100%; float: left">
		...
		<div id="storefrontTb" style="height: auto">
			<div style="margin: 5px 0 0 0;">
				<a class="easyui-linkbutton" iconCls="icon-xinzengdianmian"
					plain="true" id="addStorefrontButton" onclick="addStorefrontDlg()">新增区域</a>
				<a class="easyui-linkbutton" iconCls="icon-edit-zuke" plain="true"
					id="delStorefrontButton" onclick="uppStorefront()">修改区域</a>
			</div>
		</div>
		<div style="margin: 0 0 0 5px">
			<div style="margin: 3px 0 5px 5px; color: black; float: left">
				区域状态：<select id="searchStorefrontState"
		...
		<table id="storefrontDg"
			...
			<thead>
				<tr>
					<th field="storefrontId" width="10" align="center">区域编号</th>
					<th field="storefrontName" width="25" align="center">区域名称</th>
				</tr>
			</thead>
		</table>
		...
		<div id="userInfoTb" style="height:auto">
			<div style="margin:5px 0 0 0;">
				<a class="easyui-linkbutton" iconCls="icon-add-zuke" plain="true"
					id="addUserButton" onclick="addUserDlg()">新增用户</a>
				<a class="easyui-linkbutton" iconCls="icon-edit-zuke" plain="true"
					id="quitUserButton" onclick="employeeQuit()">普通离职</a>
				<a class="easyui-linkbutton" iconCls="icon-edit-zuke" plain="true"
					id="outUserButton" onclick="employeeTurnover()">业务员离职交接</a>
				<a class="easyui-linkbutton" iconCls="icon-add-zuke" plain="true"
				...
				<a class="easyui-linkbutton" iconCls="icon-ok" plain="true"
				   id="doFrozenButton" onclick="updateSubFrozenTrueOrFalse(0)">冻结</a>
				<a class="easyui-linkbutton" iconCls="icon-cancel" plain="true"
				   id="cancelFrozenButton" onclick="updateSubFrozenTrueOrFalse(1)">取消冻结</a>
			</div>
			...
			<div id="searchUser" style="margin: 0 0 0 5px">
				...
				<div style="margin: 3px 0 5px 5px; color: black; float: left">
					用户状态：<select id="searchSuStaffState" class="searchSuStaffState"
						onchange="queryUser()" style="width: 80px">
					</select>
				</div>
				<input style="width:0px;hight:0px;border:none"><%--此input用来处理chrome浏览器自动填充用户名的，页面加载完成后会干掉它--%>
			</div>
		</div>
		<table id="userDg"
			style="width: 100%; height: 80%; table-layout: fixed; overflow: hidden;"
			...
			<thead>
				<tr>
					...
					<th field="suName" width="10" align="center">用户账号</th>
					<th field="suChooseRoomToday" width="12" align="center">今日洗房数量</th>
					<th field="suChooseRoomLimit" width="12" align="center">每日洗房上限</th>
					<th field="suWhetherGoOut" formatter="formatSuWhetherGoOut"
						width="10" align="center">外出状态</th>
					<th field="suFrozen" formatter="formatSubFrozenTrueOrNot" width="10" align="center" >冻结状态</th>
				</tr>
			</thead>
		</table>
	...
	<div id="addStorefrontDlg" style="padding: 6px" class="easyui-dialog"
		...
		<div style='margin: 5px 0 0 0; float: left;'>
			区域名称：<input style="width: 180px" class="add_storefront_name"
				require="require">
		</div>
		...
		<div style='margin: 5px 0 0 0; float: left;'>
			区域地址：<input style="width: 180px" class="add_storefront_address"
		...
		<div style='margin: 5px 0 0 0; float: left; display: none'
			...
			区域状态：<select style="width: 180px" id="add_storefront_state"
		...
		<div style='margin: 5px 0 0 0; float: left;'>区域描述：</div>
	...
	<div id="addUserDlg" style="padding: 6px" class="easyui-dialog"
		...
		<div style="float: left;">
			<div style='margin: 10px 0 0 10px; float: left;'>
				用户状态：<select style="width:100px;" id="add_userState" class="searchSuStaffState">
				</select>
			</div>
			<div style='margin: 10px 0 0 10px; float: left;'>
			...
			<div style='margin:10px 0 0 10px;float: left;'>
				用户账号：<input style="width:96px" class="add_userAccount" needs="1"  verify="verify">
			</div>
		...
		<div style='margin: 5px 0 5px 10px; float: left;'>
			...
			<div style='margin: 5px 5px 5px 60px; float: left;'id="beinofficeinformation">
				<button style="width: 115px;height: 30px; " onclick="upastheinformation()">
				任职信息
				</button>
			</div>

			<div style="margin: 5px; float: left;"id="managementEquipment">
				<button style="width: 115px;height: 30px; " onclick="upmanagementEquipment(1)">
				管理设备
				</button>
			</div>
			<div style="margin: 5px; float: left;"id="managementEquipment1">
				<button style="width: 115px;height: 30px; " onclick="upmanagementEquipment(2)">
				管理设备
				</button>
			</div>
			<div style="margin: 5px; float: left;">
				<button  id="discountAuthBtn" style="width: 115px ;height: 30px" onclick="showAuthDlg()">
					折扣授权
				</button>
			</div>
			<div style="margin: 5px; float: left;">
		...
		<div style="margin: 10px 0 0 0; text-align: center;">
			...
			<div  id="saveUpdateUserDiv1" style="text-align: center;">
				<div id="suFrozenDiv" style="display: none;width: 250px; height: 19px;font-size: 14px;float: right;" >
					电话无跟进最大次数:
					<input id="suFrozenInput" name="suFollowupValue" readonly="readonly " style="width: 50px; border:none; color: red">
				</div>
	...
	<div id="classRoomDiv" style="padding:6px;" class="easyui-dialog" data-options="closed:true">
		...
		<div style="margin:20px 0 0 0;text-align: center;">
			...
			<a class="easyui-linkbutton" iconcls="icon-cancel" onclick="$('#classRoomDiv').dialog('close');">取消</a>
		</div>
	</div>
	...
	<div id="pushingCardDlg" style="padding:6px" class="easyui-dialog" data-options="closed:true">
		...
		<div style='margin:10px 0 0 15px;float: left;'>
			用户：<input id="selectRentan" readonly="readonly" style="width:120px"/>
		</div>
		<div style='margin:10px 0 0 15px;float: left;'>
	...
	<div id="deptTransferDlg" style="padding: 6px" class="easyui-dialog"
		...
		<fieldset>
			...
			<div style='margin: 5px 0 5px 12px; float: left;'>
				区域：<input style="width: 100px" id="originalStore"
		...
		<fieldset>
			<legend>调转部门</legend>
			<div style='margin: 5px 0 5px 12px; float: left;'>
				区域：<select id='transferStore' style="width: 100px;"
	...
	<div id="employeeTurnoverDlg" style="padding: 6px" class="easyui-dialog" data-options="closed:true">
		<fieldset>
			<legend>离职用户</legend>
			<div style='margin: 5px 0 5px 12px; float: left;'>
				区域：<input style="width: 100px" id="turnoverStore"
						  disabled="disabled">
			</div>
			<div style='margin: 5px 0 5px 12px; float: left;'>
	...
	<div id="astheinformation" style="padding:6px" class="easyui-dialog"
		...
		<fieldset>
			...
			<div style='margin:5px 0 0 24px;float: left;'>
				区域：<select style="width:80px" id="addTheStore"
		...
		<fieldset>
			...
			<div style='margin:5px 0 0 0px;float: left;'>
				<!--姓名：<input style="width:150px" class="add_userName" needs="1"> -->
				<input style="display:none" class="add_userId">
			</div>

			<!-- <div style='margin:5px 0 0 0px;float: left;'
				id="suStaffStateDiv">
				用户状态：<select style="width:150px;" id="add_userState" class="searchSuStaffState">
				</select>
			...
			<div style="clear:both"></div>
			<div style='margin:5px 0 0 0px;float: left;'>
				洗房上限：<input style="width:150px" class="add_chooseRoomNumber">
			</div>
			<div style='margin:5px 0 0 34px;float: left;'>
				：<select style="width:150px" class="add_permissions" needs="1">
					<option></option>
				</select>
			</div>
			<div style="clear:both"></div>
			<%-- <div style='margin:5px 0 0 0px;float: left;'>
				安全级别：<select style="width:150px" class="add_checkType" needs=1>
					<option value="0">0</option>
					<option value="1">1</option>
					<option value="2">2</option>
					<option value="3">3</option>
			...
			<div style='margin:5px 0 0 10px;float: left;'>
				安全加密：<input style="width:150px" class="add_check" >
			</div>
			<div style="clear:both"></div>
			<div style='margin:5px 0 0 0px;float: left;'>
				用户账号：<input style="width:150px" class="add_userAccount" needs="1">
			</div>
			<div style='margin:5px 0 0 10px;float: left;'>
				登录密码：<input style="width:150px" class="add_userPassword" type="password" needs="1">
				<a class="easyui-linkbutton" id="showPassword" onclick="showPassword()" >显示密码</a>
			</div>
			<div style="clear: both;"></div> --%>
			<div style='margin:5px 0 0 0;float: left;'>
	...
	<script src="js/fg.user.js"></script>
